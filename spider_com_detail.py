...
def parse_htmls(html):
    while True:
        try:
            ...
            com_capital = re.findall('<td>[^<]*<span>注册资本:[^<]*</span>[^<]*</td>[^<]*<td>[^<]*<span>([^<]*)</span>', html)[0]
            print com_capital

            com_address = re.findall('<td>[^<]*<span>注册地址:[^<]*</span>[^<]*</td>[^<]*<td>[^<]*<span>([^<]*)</span>', html)[0]
            print com_address

            legal_person = re.findall('<td>[^<]*<span>法人代表:[^<]*</span>[^<]*</td>[^<]*<td>[^<]*<span>([^<]*)</span>', html)[0]
            print legal_person

            create_time = re.findall('<td>[^<]*<span>成立时间:[^<]*</span>[^<]*</td>[^<]*<td>[^<]*<span>([^<]*)</span>', html)[0]
            print create_time

            contact_info = re.findall('<td>[^<]*<span>官方联系方式:[^<]*</span>[^<]*</td>[^<]*<td>[^<]*<span>([^<]*)</span>', html)[
                0]
            print contact_info
...
#     cursor = conn.cursor()
#
#     html = 'https://www.innotree.cn/inno/company/10286690239861118073.html'
#     content = get_parse(html)
#
#     cursor.execute(
#         'insert into innotree_company_detailInfo values("%s","%s","%s","%s","%s","%s","%s","%s","%s","%s")' % tuple(
